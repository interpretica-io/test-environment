...
# Create a bare repository $1 with one commit per extra argument;
# every commit is tagged v<n> and the file content is the argument.
mk_repo() {
    local bare="$1"; shift
...
# Write a processed builder conf declaring a single repository
# named 'ext' with URL $1 and ref $2.
mk_conf() {
    local conf="$3"
...
# Run the fetcher; stdout+stderr go to ${WORK}/out, exit code to $?
run_fetch() {
